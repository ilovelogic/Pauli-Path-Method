...
class PauliPathTraversal:
    ...
    def __init__(self, num_qubits:int, weight_combo:List[int],gate_pos:List[List[tuple]]):
        ...
        if (min_index-1 >= 0):
            min_next_sibs_b = self.propagate_next(min_layer_ops.backward_sibs, pos_to_fill_b, 1, min_index)
            if (min_index-2 >= 0):
                self.layers[min_index-1] = PauliOpLayer(gate_pos[min_index-2], 1, min_next_sibs_b)
            else:
                self.layers[min_index-1] = PauliOpLayer()
                self.layers[min_index-1].forward_sibs = min_next_sibs_b
        
        if (min_index < self.num_op_layers-1):
            min_next_sibs_f = self.propagate_next(self.layers[min_index].forward_sibs, pos_to_fill_f, 0, min_index)
        ...
        for i in range(min_index+2, self.num_op_layers): # Goes from min_index + 2 to self.num_op_layers-1
            ...
            self.layers[i] = PauliOpLayer(gate_pos[min_index+1], 0, next_sibs_f)
    ...
    Args:
        all_sibs (DefaultDict[tuple, List[PauliOperator]]) : the hash map of all sibling operators at a neighboring layer of the Pauli path
        pos_to_fill (DefaultDict[PauliOperator,List]) : a hash map of the gate positions with non-identity I/O between the new operator layer
        and the layer we are propagating to
        backward (int) : 1 if we are propagating backward, 0 if we are propagating forward
        index (int) : the new index in the Pauli path for which we are getting sibling operators

    Returns:
        DefaultDict[tuple,List[PauliOperator]]: a list of all the sibling operators that the prior index's 
    ...
    def propagate_next(self, all_sibs:DefaultDict[tuple, List[PauliOperator]], pos_to_fill:DefaultDict[PauliOperator,List], backward:int, index:int):
        ...
        for sib_ops in all_sibs.values(): # Iterates over each sibling operators list at the former index 
            ...
            sib_ops[0].weight_to_operators(self.weight_combo[index-backward], copy.deepcopy(pos_to_fill[sib_ops[0]]), backward)
        if (backward):
